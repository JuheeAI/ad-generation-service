    ...
    def run(self, inputs):
        ...
        try:
            ...
            try:
                ...
                inputs["product_category"] = "other"

            background_input = params.get("background", "화이트(기본)")
            bg_key_map = {
                "화이트(기본)": "white_default",
                "매장 내부": "store_indoor",
                "야외 풍경": "outdoor_nature",
                "카페 분위기": "cafe_mood",
                "고급스러운 배경": "luxury_mood",
                "밝고 화사한 배경": "bright_mood",
                "심플 단색 배경": "simple_color"
            }
            template_id = bg_key_map.get(background_input, "white_default")
            ...
            # ==========================================
            # Case A: 텍스트만 있는 경우 (Text-to-Image)
            # ==========================================
            ...
            # ==========================================
            # Case B: 이미지가 있는 경우 (Composite Generation)
            # ==========================================
            else:
                self.logger.info("Running in AI Auto-Layout mode.")
                
                # B-1. 배경 생성 (Base Model)
                background_prompt = template.get("background_prompt", "clean professional studio background")
                ...
                # B-3. 이미지 합성 (Product + Model + Background)
                relative_scale = 0.55 # LLM으로 구하는 로직이 있으면 교체 가능
            ...
            # 3. Refiner (공통)
            self.logger.info("Running Refiner...")
            
            # VRAM 정리를 위해 이전 모델 언로드
            self.model_manager.unload("pipe_base", "pipe_controlnet", "controlnet_canny")
            if torch.cuda.is_available(): torch.cuda.empty_cache()
